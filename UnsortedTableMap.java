...
public class UnsortedTableMap<K,V> extends AbstractMap<K,V> {
  ...
  /**
   * returns the Key stored in the UnsortedTableMap
   * @return
   */
  public K getKey()
  {
  ...
  /**
   * Compares the keys of the bucket to see if each string is the same.
   * Returns are string if the two strings are not equal
   * Uses the Equals method found in the StringTwo class.
   * @return
   */
  public  String compareKey()
  {
	  ...
	  {
		  for(MapEntry<K,V> y : table)
		  {
			  if(!((StringTwo) x.getKey()).Equals((StringTwo) y.getKey()))
			  {
				  return ((StringTwo) x.getKey()).getString() + " " + ((StringTwo) y.getKey()).getString(); 
